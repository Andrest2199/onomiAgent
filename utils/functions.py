...
#     client, run, thread_id, company, employee_number, is_admin=""
# ):
...
#             logging.warning(
#                 f"%s|%s| FUNCTION NAME: {function_name} NO FUNCTION DEFINE",
#                 employee_number,
#                 company,
#             )
...
#             logging.info(
#                 f"%s|%s| API RESPONSE: {response_data}", employee_number, company
#             )
#             # Response returned to the assistant
#             tool_outputs.append({"tool_call_id": tool.id, "output": str(response_data)})
#         elif function_name == "obtener_todos_los_empleados" and not is_admin:
#             logging.info(
#                 f"%s|%s| API RESPONSE: The user doesn't have access to this information.",
#                 employee_number,
#                 company,
#             )
#             # Response returned to the assistant
...
#             if not response_data:
#                 response_data = (
#                     "No data for this employee or no data returned from the API."
#                 )
...
#                 logging.info(
#                     f"%s|%s| NEED PARAMS: {response_data}", employee_number, company
#                 )
#             else:
#                 period = params["period"]
...
#                     logging.warning(
#                         f"%s|%s| INVALID PERIOD FORMAT: {period}",
#                         employee_number,
#                         company,
#                     )
#                 else:
#                     logging.info(
#                         f"%s|%s| FUNCTION ARGS: {params}", employee_number, company
#                     )
#                     # Execute API call
#                     response_data = get_payroll_receipt(
#                         company, employee_number, period
#                     )
#                     if not response_data:
#                         response_data = (
#                             "No data for this period or no data returned from the API."
#                         )

#                     logging.info(
#                         f"%s|%s| API RESPONSE: {response_data}",
#                         employee_number,
#                         company,
#                     )
#             # Response returned to the assistant
...
#             )
#             logging.info(
#                 f"%s|%s| TOOL OUTPUTS SUBMITTED SUCCESSFULLY.", employee_number, company
#             )
#             return run
#         except Exception as e:
#             logging.error(
#                 f"%s|%s| FAILED TO SUBMIT TOOL OUTPUTS: {e}", employee_number, company
#             )
...
#                 )
#                 logging.info(
#                     f"%s|%s| RUN CANCELLED DUE TO SUBMIT ERROR.",
#                     employee_number,
#                     company,
#                 )
#                 return run
...
#                 logging.error(
#                     f"%s|%s| FAILED TO CANCEL RUN: {cancel_error}",
#                     employee_number,
#                     company,
#                 )
...
#             return {
#                 "status": "error",
#                 "message": "Sorry for the inconvenience. At the moment, the answer is not available. If you need immediate assistance, please contact your Human Resources department directly.",
#             }
#     else:
...
#             logging.info(
#                 f"%s|%s| RUN CANCELLED DUE TO NO TOOL OUTPUTS.",
#                 employee_number,
#                 company,
#             )
#             return run
#         except Exception as cancel_error:
#             logging.error(
#                 f"%s|%s| FAILED TO CANCEL RUN: {cancel_error}", employee_number, company
#             )
#             # Return an error message to the main loop
#             return {
#                 "status": "error",
